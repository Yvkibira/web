...
head.appendChild(scriptElement);
    ...
    var check;
    function scriptAdd(){for(var i = 0; i < excludedpages.length; i++){var check =excludedpages[i]+'.html';var loc=window.location.pathname; if(loc === check || loc ==="/"){console.log(check); shouldAppendScript = false;break;}else{console.log("not "+check +"but is "+loc); shouldAppendScript = true;}}if (shouldAppendScript) {var script = document.createElement('script');script.type = 'text/javascript';script.src = 'banner.js';document.head.appendChild(script);}}window.addEventListener("load",scriptAdd);
